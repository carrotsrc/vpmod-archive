 ...
 */
	class vp_usermanComponent extends Component
	{
		private $resManager;
		private $plib;
		...
		public function createInstance($params = null)
		{
			$this->resManager = Managers::ResourceManager();
			$id = 1;
			if(($r = $this->resManager->queryAssoc("Instance()<Component('vp_userman');"))) {
				$id = sizeof($r[0])+1;
			}
		...
		private function getDetails($args)
		{
			...
			$account = $ulib->getAccount($res[0]['ref']);
			if(!$account) {
				$res[0]['email'] = "";
			} else
				$res[0]['email'] = $account[0]['email'];
